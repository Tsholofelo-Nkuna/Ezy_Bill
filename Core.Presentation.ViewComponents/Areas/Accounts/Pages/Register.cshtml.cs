                ...
                if (apiResponse is { IsSuccessStatusCode: true })
                {
                   ...
                   var response =  await this._httpClient.PostAsJsonAsync($"{ApiEndPointConstants.CreateProfile}/{newUser.Id}", userProfile);
                        ...
                        )
                    {
                        ...
                        await _emailSender.SendConfirmationLinkAsync(newUser, newUser.Email, $"{this.HttpContext.Request.Scheme}://{this.HttpContext.Request.Host}/Accounts/confirmemail{qBuilder.Value}");
